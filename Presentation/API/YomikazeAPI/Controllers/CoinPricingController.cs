    ...
    : CrudControllerBase<CoinPricing, CoinPricingModel, CoinPricingRepository>(repository, mapper, cache, logger)
{
    ...
    public override ActionResult<CoinPricingModel> Post(CoinPricingModel input)
    {
        ...
        Price? result = PriceService.Create(options);
    ...
    public ActionResult<CheckoutResultModel> Checkout([FromBody] CheckoutModel model)
    {
        ...
        SessionCreateOptions opts = new SessionCreateOptions
        {
            ...
            Metadata = new Dictionary<string, string> { ["UserId"] = uid, ["PriceId"] = priceId.ToString() },
            Mode = "payment",
            ReturnUrl = url.ToString().Replace("CHECKOUT_SESSION_ID", "{CHECKOUT_SESSION_ID}")
        };
        Session session = SessionService.Create(opts);
        return CreatedAtAction("CheckoutStatus", new { sessionId = session.Id },
            new CheckoutResultModel { SessionId = session.Id, ClientSecret = session.ClientSecret });
    }
    ...
    [Authorize]
    public ActionResult<PaymentSheetResultModel> CreatePaymentSheet([FromBody] PaymentSheetModel model,
        [FromServices] PaymentIntentService paymentIntentService, [FromServices] UserManager<User> userManager)
    {
        if (!ulong.TryParse(model.PriceId, out ulong priceId))
        {
            ModelState.AddModelError(nameof(model.PriceId), "Invalid price ID.");
        ...
        PaymentIntentCreateOptions paymentIntentOptions = new PaymentIntentCreateOptions
        {
            ...
            {
                ["UserId"] = User.GetIdString(), ["PriceId"] = pricing.Id.ToString()
            }
        };
        ...
        [FromServices] UserManager<User> userManager)
    {
        ...
        Transaction transaction = new()
        {
            ...
            Type = TransactionType.PurchaseCoin
        };
